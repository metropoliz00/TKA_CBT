        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 fade-in p-6 print:shadow-none print:border-0 print:p-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 print:hidden">
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    ...
                    <button onClick={handlePrint} className="bg-rose-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2 hover:bg-rose-700 transition shadow-lg shadow-rose-200"><Printer size={16}/> Cetak</button>
            ...
            <div className="hidden print:block mb-4 text-center">
                <h2 className="text-xl font-bold uppercase">Peringkat Peserta TKA</h2>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-200 print:border-0">
                <table className="w-full text-sm text-left print:text-xs">
                    <thead className="bg-slate-50 font-bold text-slate-600 uppercase text-xs print:bg-white print:border-b-2 print:border-black">
                        <tr>
                            <th className="p-4 text-center w-16 print:p-2">Rank</th>
                            <th className="p-4 print:p-2">Username</th>
                            <th className="p-4 print:p-2">Nama</th>
                            <th className="p-4 print:p-2">Sekolah</th>
                            <th className="p-4 print:p-2">Kecamatan</th>
                            <th className="p-4 text-center bg-blue-50/50 border-l border-slate-200 print:bg-white print:p-2">B. Indo</th>
                            <th className="p-4 text-center bg-orange-50/50 border-l border-slate-200 print:bg-white print:p-2">Mat</th>
                            <th className="p-4 text-center border-l border-slate-200 print:p-2">Akhir</th>
                            <th className="p-4 text-center border-l border-slate-200 print:p-2">Predikat</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 print:divide-slate-300">
                        {loading ? (
                            ...
                            filteredData.map((d, i) => (
                                <tr key={i} className="border-b hover:bg-slate-50 transition print:hover:bg-transparent">
                                    <td className="p-4 font-bold text-center text-slate-500 print:p-2"><div className={`w-8 h-8 rounded-full flex items-center justify-center mx-auto ${i < 3 ? 'bg-yellow-100 text-yellow-700 font-black print:bg-transparent print:text-black print:border print:border-black' : 'bg-slate-100 print:bg-transparent'}`}>{i+1}</div></td>
                                    <td className="p-4 font-mono text-slate-600 print:p-2">{d.username}</td>
                                    <td className="p-4 font-bold text-slate-600 print:p-2">{d.nama}</td>
                                    <td className="p-4 text-slate-600 print:p-2">{d.sekolah}</td>
                                    <td className="p-4 text-slate-600 print:p-2">{d.kecamatan}</td>
                                    <td className="p-4 text-center font-bold text-blue-600 bg-blue-50/10 border-l border-slate-100 print:bg-transparent print:text-black print:p-2">{d.score_bi !== null ? d.score_bi : '-'}</td>
                                    <td className="p-4 text-center font-bold text-orange-500 bg-orange-50/10 border-l border-slate-100 print:bg-transparent print:text-black print:p-2">{d.score_mtk !== null ? d.score_mtk : '-'}</td>
                                    <td className="p-4 text-center font-extrabold text-indigo-600 text-lg border-l border-slate-100 print:text-black print:p-2">{d.avg.toFixed(2)}</td>
                                    <td className="p-4 text-center border-l border-slate-100 print:p-2">{getPredicateBadge(d.avg)}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
